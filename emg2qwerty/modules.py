...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, num_features: int, max_len: int = 10000, dropout: float = 0.1) -> None:
        ...
        div_term = torch.exp(
            torch.arange(0, num_features, 2).float() * (-torch.log(torch.tensor(10000.0)) / num_features)
        )
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(1)  # (max_len, 1, num_features)
...
class TransformerEncoder(nn.Module):
    """Transformer encoder for temporal EMG data.
    ...
    Args:
        num_features: Feature dimension from MultiBandRotationInvariantMLP.
    ...
    def __init__(
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=False,  # input is (T, N, d_model)
        )
